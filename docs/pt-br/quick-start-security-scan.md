...
O padrão de nome é `security-scan-{versão}-{timestamp}-{plataforma}`. Baixe sempre o binário da release mais recente.
...
```powershell
.\security-scan-win-x64.exe --version
```
...
Diretório de saída dos relatórios: [.security-scan/reports]
```

Se habilitado, o `fix` salva relatórios em Markdown no diretório informado. O padrão `.security-scan/reports` funciona bem para a maioria dos casos.
...
Próximos passos:
  ...
  3. Execute: security-scan scan --cwd <diretório-do-projeto>
```
...
Agora o `security-scan fix` vai conectar ao SonarQube local em vez de tentar subir um container gerenciado.
...
| Comando | O que faz |
|---|---|
| `security-scan init` | Gera o `project-config.yml` via assistente interativo |
| `security-scan init --force` | Sobrescreve o `project-config.yml` existente |
| `security-scan scan` | Varre vulnerabilidades (somente leitura, sem modificar arquivos) |
| `security-scan fix` | Pipeline completo: scan → atualizar → validar → relatório |
| `security-scan fix --dry-run` | Simula o pipeline sem modificar nada |
| `security-scan fix --authorize-breaking npm` | Fix autorizando breaking changes em npm |
| `security-scan fix --phases scan,npm` | Roda apenas as fases informadas |
| `security-scan --help` | Exibe ajuda geral |
| `security-scan fix --help` | Exibe ajuda do comando fix |

---

> Para configurações avançadas (protected_packages, safe_update_policy, cloud storage, CI/CD), consulte o [Guia Completo de Uso](./usage-guide-security-scan.md).
