...
export class ComposerDockerRunner implements EphemeralContainerRunner<string[]> {
  ...
  async runShell(command: string, opts?: { cwd?: string }): Promise<ContainerRunResult> {
    ...
    logger.debug(`ComposerDockerRunner (shell): docker ${dockerArgs.join(' ')}`);
    ...
    try {
      ...
      const e = err as { exitCode?: number; stdout?: string; stderr?: string; message?: string };
      logger.debug(`ComposerDockerRunner (shell): container exited ${typeof e.exitCode === 'number' ? e.exitCode : 1}`);
      containerResult = {
        exitCode: typeof e.exitCode === 'number' ? e.exitCode : 1,
        stdout: e.stdout ?? '',
        stderr: e.stderr ?? e.message ?? String(err),
