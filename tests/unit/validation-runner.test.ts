...
function makeRunner(
  ...
  return {
    ...
    run: vi.fn(async (command: string) => {
      ...
      return { stdout: '', stderr: '', exitCode: defaultExitCode, command, dryRun: false };
    }),
...
describe('runValidations', () => {
  ...
  it('returns pass entry when command exits 0', async () => {
    const runner = makeRunner({ 'npm run build': { stdout: 'Build complete', exitCode: 0 } });
  ...
  it('returns fail entry and allPassed=false when command exits non-zero', async () => {
    const runner = makeRunner({
      'npm run build': { stdout: 'error output', stderr: 'build error', exitCode: 1 },
    });
  ...
  it('falls back to exit code message when both stdout and stderr empty on failure', async () => {
    const runner = makeRunner({ 'npm run lint': { stdout: '', stderr: '', exitCode: 127 } });
  ...
  it('logs command, stdout, stderr and exit code on failure', async () => {
    const runner = makeRunner({
      'npm run build': { stdout: 'some output', stderr: 'some error', exitCode: 1 },
    });
    ...
    const errorCalls: string[] = (logger.error as ReturnType<typeof vi.fn>).mock.calls.map(
  ...
  it('does not log stdout/stderr sections when they are empty on failure', async () => {
    const runner = makeRunner({ 'npm run lint': { stdout: '', stderr: '', exitCode: 1 } });
